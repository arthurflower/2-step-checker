...
export class ContentAnalyzer {
  ...
  static analyze(content: string): ContentAnalysis {
    ...
    if (typeof content !== "string") {
      console.error(
        "ContentAnalyzer.analyze received non-string input:",
        content,
      );
      return {
        wordCount: 0,
        ...
        warnings: ["Invalid content input to analyze."],
        canProcess: false,
    ...
    const averageWordsPerSentence =
      sentenceCount > 0
        ? parseFloat((wordCount / sentenceCount).toFixed(1))
        : 0;
    ...
    const estimatedClaims =
      sentenceCount > 2
        ? Math.ceil(sentenceCount * this.LIMITS.AVG_CLAIMS_PER_SENTENCE)
        : Math.ceil(
            wordCount /
              (averageWordsPerSentence > 5 ? averageWordsPerSentence : 15),
          );
    const estimatedProcessingTime = Math.max(
      5,
      estimatedClaims * this.LIMITS.PROCESSING_TIME_PER_CLAIM,
    ); // Min 5 seconds
    ...
    if (wordCount < this.LIMITS.MIN_WORDS_FOR_PROCESSING) {
      ...
      canProcess = false;
      warnings.push(
        `Content is too short. Minimum ${this.LIMITS.MIN_WORDS_FOR_PROCESSING} words required.`,
      );
    } else if (wordCount > this.LIMITS.MAX_WORDS_TOTAL) {
      processingStrategy = "too-large";
      canProcess = false;
      warnings.push(
        `Content exceeds maximum limit of ${this.LIMITS.MAX_WORDS_TOTAL.toLocaleString()} words (approx. ${Math.round(this.LIMITS.MAX_WORDS_TOTAL / this.LIMITS.WORDS_PER_PAGE)} pages). Your content has ${wordCount.toLocaleString()} words.`,
      );
    } else if (
      wordCount > this.LIMITS.MAX_WORDS_PER_REQUEST ||
      characterCount > this.LIMITS.MAX_CHARS_PER_REQUEST
    ) {
      processingStrategy = "chunked";
      warnings.push(
        "Content will be processed in multiple chunks for better performance and accuracy due to its size.",
      );
    }

    if (estimatedProcessingTime > 120) {
      // 2 minutes
      const timeInMinutes = Math.ceil(estimatedProcessingTime / 60);
      warnings.push(
        `Estimated processing time: ~${timeInMinutes} minutes. This may vary.`,
      );
    } else if (estimatedProcessingTime > 30) {
      warnings.push(
        `Estimated processing time: ~${Math.ceil(estimatedProcessingTime)} seconds.`,
      );
    }
    ...
    if (
      ...
      // 5 minutes
      warnings.push(
        "For very large documents, consider processing smaller sections if this takes too long.",
      );
    }
    ...
    if (processingStrategy === "chunked" && canProcess) {
      ...
      if (chunks.length > this.LIMITS.MAX_CHUNKS) {
        warnings.push(
          `Document split into ${chunks.length} chunks, exceeding the processing limit of ${this.LIMITS.MAX_CHUNKS} chunks. Only the first ${this.LIMITS.MAX_CHUNKS} will be processed.`,
        );
    ...
    return {
      ...
      canProcess,
      topic: undefined,
      category: undefined, // These will be populated by the extraction API
    };
  }
  ...
  private static createChunks(
    ...
    const targetSentencesPerChunk = Math.max(
      10,
      Math.ceil(
        sentences.length / (this.LIMITS.MAX_WORDS_TOTAL / maxWordsPerChunk),
      ),
    );
    ...
    if (preferSentenceChunking && sentences.length > 0) {
      ...
      while (currentSentenceNum < sentences.length) {
        ...
        let chunkStartIndexInFullDoc = content.indexOf(
          sentences[currentSentenceNum],
          currentGlobalCharIndex,
        );
        if (chunkStartIndexInFullDoc === -1 && currentSentenceNum > 0) {
          // Safety if sentence not found, try after previous chunk
          chunkStartIndexInFullDoc = content.indexOf(
            sentences[currentSentenceNum],
            chunks[chunks.length - 1].endIndex,
          );
        }
        if (chunkStartIndexInFullDoc === -1)
          chunkStartIndexInFullDoc = currentGlobalCharIndex; // Fallback
        ...
        for (let i = currentSentenceNum; i < sentences.length; i++) {
          ...
          if (
            chunkWordCount + sentenceWords > maxWordsPerChunk ||
            chunkContent.length + sentence.length > maxCharsPerChunk ||
            chunkSentenceCount >= targetSentencesPerChunk * 1.5
          ) {
            // Added a bit of leeway for target sentences
            if (chunkSentenceCount === 0) {
              // Ensure at least one sentence if it's too large itself
          ...
          currentSentenceNum = i + 1;
          if (
            chunkSentenceCount >= targetSentencesPerChunk &&
            chunkWordCount > maxWordsPerChunk * 0.7
          )
            break; // Break if target met and substantial size
        }

        if (chunkSentenceCount > 0) {
          const chunkEndIndexInFullDoc =
            chunkStartIndexInFullDoc + chunkContent.length;
          chunks.push({
            id: `chunk-s-${chunks.length + 1}`,
            content: chunkContent,
            wordCount: chunkWordCount,
            sentenceCount: chunkSentenceCount,
            startIndex: chunkStartIndexInFullDoc,
            endIndex: chunkEndIndexInFullDoc,
          });
      ...
      while (currentWordNum < words.length) {
        ...
        for (let i = currentWordNum; i < words.length; i++) {
          const word = words[i];
          if (
            currentChunkWordCount + 1 > maxWordsPerChunk ||
            currentChunkCharCount + word.length + 1 > maxCharsPerChunk
          ) {
        ...
        if (currentChunkWordCount > 0) {
          ...
          let actualStartIndex = content.indexOf(
            finalChunkContent,
            currentGlobalCharIndex - finalChunkContent.length > 0
              ? currentGlobalCharIndex - finalChunkContent.length
              : 0,
          ); // Search in a window
          if (actualStartIndex === -1)
            actualStartIndex = content.indexOf(
              chunkContentWords[0],
              currentGlobalCharIndex,
            );
          if (actualStartIndex === -1)
            actualStartIndex = chunkStartIndexInFullDoc; // Fallback

          const chunkSentences = this.getSentences(finalChunkContent);
          chunks.push({
            id: `chunk-w-${chunks.length + 1}`,
            content: finalChunkContent,
            wordCount: currentChunkWordCount,
            sentenceCount: chunkSentences.length,
            startIndex: actualStartIndex,
            endIndex: actualStartIndex + finalChunkContent.length,
          });
    ...
    if (chunks.length > 1) {
      ...
      if (
        lastChunk.wordCount < maxWordsPerChunk * 0.1 &&
        chunks.length <= this.LIMITS.MAX_CHUNKS
      ) {
        // If last chunk is tiny
        const secondLastChunk = chunks[chunks.length - 2];
        if (
          secondLastChunk.wordCount + lastChunk.wordCount <= maxWordsPerChunk &&
          secondLastChunk.content.length + lastChunk.content.length <=
            maxCharsPerChunk
        ) {
  ...
  static estimateCost(analysis: ContentAnalysis): {
    ...
    const extractionCalls =
      analysis.processingStrategy === "chunked" && analysis.chunks
    ...
    const avgCharsPerExtractionChunk =
      analysis.characterCount / extractionCalls;
    const costPerExtractionCall =
      (avgCharsPerExtractionChunk / 1000) * 0.000125 +
      (avgCharsPerExtractionChunk / 2000) * 0.000375; // Simplified
    const costPerVerificationCall =
      (1000 / 1000) * 0.000125 + (500 / 1000) * 0.000375; // Assuming ~1k char input, 0.5k output for verification

    const serperCostPerCall = 0.001; // Assuming $1 per 1000 searches

    const totalGeminiCost =
      extractionCalls * costPerExtractionCall +
      verificationCalls * costPerVerificationCall;
...
if (typeof ContentAnalyzer !== "undefined") {
  ...
  console.error(
    "ContentAnalyzer class is UNDEFINED at the end of contentAnalyzer.ts (v2)!",
  );
}
