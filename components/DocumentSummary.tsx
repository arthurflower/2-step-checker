...
const StatDisplay: React.FC<{
  ...
  >
    <div className={`mb-1.5 ${colorClass || "text-accent-primary"}`}>
      {icon}
    </div>
    <div
      className={`font-bold ${colorClass} ${size === "large" ? "text-xl md:text-2xl" : "text-lg md:text-xl"}`}
    >
      {value === undefined ||
      value === null ||
      value === "" ||
      value === "N/A (Subsequent Chunk)" ? (
    ...
    <div
      className={`text-xs text-text-muted ${size === "large" ? "mt-0.5" : "mt-0.5"}`}
    >
      {label}
    </div>
    {tooltip && (
...
const VerificationCheckDisplay: React.FC<{
  title: string;
  ...
  const reasonText =
    check && check.reason ? check.reason : "No specific reason provided.";
  const verdictText = check && check.verdict ? check.verdict : "N/A";
  const scoreText =
    check && check.score !== undefined && check.score !== null
      ? ` (${check.score}/100)`
      : "";
  ...
  return (
    <div className="p-3 bg-bg-secondary rounded-lg border border-surface-border">
      <div className="flex items-center justify-between mb-1">
        ...
        <span
          className={`text-xs font-bold ${getVerdictColor(verdictText as VerificationCheck["verdict"])} flex items-center gap-1`}
        >
          {getVerdictIcon(verdictText as VerificationCheck["verdict"])}{" "}
          {verdictText}
          {scoreText}
        </span>
      </div>
...
const DocumentSummary: React.FC<DocumentSummaryProps> = ({
    ...
    return (
      <div className="w-full mb-10 opacity-0 animate-fade-up [animation-delay:200ms]">
        <div className="summary-card p-6 md:p-8 glass-card">
          <div className="h-10 bg-bg-secondary rounded w-3/4 shimmer mb-4"></div>{" "}
          {/* Title placeholder */}
          <div className="h-20 bg-bg-secondary rounded-lg shimmer mb-6"></div>{" "}
          {/* Overall status placeholder */}
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
            {[...Array(6)].map((_, i) => (
              <div
                key={i}
                className="h-24 bg-bg-secondary rounded-lg shimmer"
              ></div>
            ))}
          </div>
  ...
  if (!currentContentAnalysis) {
    return (
      <div className="w-full mb-10 text-center text-text-muted p-6 summary-card glass-card">
        Document analysis data is not available. Please process a document.
      </div>
    );
  }

  const goCount = results.filter(
    (r) => r.multi_dimensional_verification?.final_verdict === "GO",
  ).length;
  const checkCount = results.filter(
    (r) => r.multi_dimensional_verification?.final_verdict === "CHECK",
  ).length;
  const noGoCount = results.filter(
    (r) => r.multi_dimensional_verification?.final_verdict === "NO GO",
  ).length;
    ...
    totalVerifiedClaims > 0
      ? Math.round(
          results.reduce(
            (sum, r) =>
              sum +
              (r.multi_dimensional_verification?.confidence_calibration
                ?.score || 0),
            0,
          ) / totalVerifiedClaims,
        )
  ...
  let overallStatusDescription =
    "The document has not been fully processed or no verifiable claims were found.";
  ...
  if (totalVerifiedClaims > 0) {
    if (noGoCount > 0 || checkCount > totalVerifiedClaims * 0.3) {
      ...
      overallStatusDescription =
        'All verified claims were assessed as "GO". The document appears factually sound based on the analysis.';
    }
  } else if (
    extractionData &&
    extractionData.totalClaimsExtracted === 0 &&
    !isLoading
  ) {
    ...
    overallStatusDescription =
      "No verifiable claims were extracted from the document for analysis.";
  }

  const avgSourceAuthority =
    results.length > 0 &&
    results[0].multi_dimensional_verification?.enhanced_source_summary
      ?.source_authority_score?.score !== undefined
      ? Math.round(
          results.reduce(
            (sum, r) =>
              sum +
              (r.multi_dimensional_verification?.enhanced_source_summary
                ?.source_authority_score?.score || 0),
            0,
          ) / results.length,
        )
  ...
  const defaultMdvCheck: VerificationCheck = {
    verdict: "N/A",
  ...
  return (
    <div className="w-full mb-10 opacity-0 animate-fade-up [animation-delay:200ms]">
      <div className="summary-card p-6 md:p-8 glass-card">
        ...
        <div
          className={`p-5 md:p-6 rounded-xl mb-8 border bg-opacity-10 ${
            overallStatusColor === "text-success"
        ...
        >
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4">
            <OverallStatusIconComp
              size={36}
              className={`${overallStatusColor} flex-shrink-0`}
            />
            <div>
              <h3
                className={`text-lg md:text-xl font-semibold ${overallStatusColor}`}
              >
                {overallStatusText}
              </h3>
          ...
          {totalVerifiedClaims > 0 && (
            <div className="mt-4 pt-3 border-t border-current/20 flex flex-wrap gap-x-6 gap-y-2 text-xs">
              <span className="flex items-center gap-1.5">
                <ThumbsUp size={14} className="text-success" /> GO: {goCount}
              </span>
              <span className="flex items-center gap-1.5">
                <HelpCircle size={14} className="text-warning" /> CHECK:{" "}
                {checkCount}
              </span>
              <span className="flex items-center gap-1.5">
                <ThumbsDown size={14} className="text-error" /> NO GO:{" "}
                {noGoCount}
              </span>
              <span className="flex items-center gap-1.5">
                <Gauge size={14} className={overallStatusColor} /> Avg.
                Confidence: {overallAccuracyScore}%
              </span>
            </div>
          )}
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 gap-3 md:gap-4 text-sm">
            <StatDisplay
              ...
              label="Topic"
              value={extractionData?.topic || currentContentAnalysis.topic}
              size="large"
              tooltip={extractionData?.topic || currentContentAnalysis.topic}
            />
            <StatDisplay
              icon={<Tag size={20} />}
              label="Category"
              value={
                extractionData?.category || currentContentAnalysis.category
              }
              size="large"
              tooltip={
                extractionData?.category || currentContentAnalysis.category
            ...
            <StatDisplay
              ...
              label="Doc Type"
              value={extractionData?.document_type}
              size="large"
              tooltip={extractionData?.document_type}
            />
            <StatDisplay
              icon={<BarChart3 size={20} />}
              label="Words"
              value={currentContentAnalysis.wordCount?.toLocaleString()}
            />
            <StatDisplay
              icon={<ListChecks size={20} />}
              label="Sentences"
              value={currentContentAnalysis.sentenceCount?.toLocaleString()}
            />
            <StatDisplay
              icon={<Brain size={20} />}
              label="Claims Extracted"
              value={extractionData?.totalClaimsExtracted ?? "N/A"}
            />
            <StatDisplay
              icon={<Users size={20} />}
              label="Words/Sentence"
              value={currentContentAnalysis.averageWordsPerSentence}
            />
            <StatDisplay
              icon={<Hash size={20} />}
              label="References (doc)"
              value={
                extractionData?.references_in_doc_count === 0
                  ? "0"
                  : extractionData?.references_in_doc_count
              }
              tooltip="References cited within the document body."
            />
            <StatDisplay
              icon={<TrendingUp size={20} />}
              label="Typical Accuracy"
              value={extractionData?.expected_accuracy_range}
              tooltip="Expected accuracy range for this document type and category."
            />
          </div>
        </div>
        ...
        {results.length > 0 && results[0].multi_dimensional_verification && (
          <div className="mb-6">
            <h4 className="text-sm font-semibold text-text-secondary uppercase tracking-wider mb-3">
              Overall Source Quality (Average for Verified Claims)
            </h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
              <VerificationCheckDisplay
                title="Avg. Source Authority"
                check={{
                  ...(results[0].multi_dimensional_verification
                    .enhanced_source_summary.source_authority_score ||
                    defaultMdvCheck),
                  score: avgSourceAuthority,
                  reason: `Average authority score of sources used for verified claims. Higher scores indicate more reputable sources.`,
                }}
              ...
              <VerificationCheckDisplay
                ...
                check={
                  results[0].multi_dimensional_verification
                    .enhanced_source_summary.source_freshness_index ||
                  defaultMdvCheck
                }
              ...
              <VerificationCheckDisplay
                ...
                check={
                  results[0].multi_dimensional_verification
                    .enhanced_source_summary.source_bias_detection ||
                  defaultMdvCheck
                }
                icon={<Scale size={14} />}
              />
              <VerificationCheckDisplay
                ...
                check={
                  results[0].multi_dimensional_verification
                    .enhanced_source_summary.source_consensus_mapping ||
                  defaultMdvCheck
                }
                icon={<Link2 size={14} />}
              />
        ...
        {(extractionData?.funding_source &&
          extractionData.funding_source !== "Not Mentioned") ||
        (extractionData?.author_credibility &&
          extractionData.author_credibility !==
            "No specific author information to assess") ? (
          <div className="mt-6 pt-4 border-t border-surface-border">
            <h4 className="text-sm font-semibold text-text-secondary uppercase tracking-wider mb-3">
              ...
              {extractionData?.funding_source &&
                extractionData.funding_source !== "Not Mentioned" && (
                  <div className="p-3 bg-bg-secondary rounded-md border border-surface-border">
                    <span className="text-text-muted font-medium block text-xs mb-0.5">
        ...
        {currentContentAnalysis.warnings &&
          currentContentAnalysis.warnings.length > 0 && (
            <div className="mt-6 pt-4 border-t border-surface-border">
              ...
              <ul className="list-disc list-inside text-xs text-warning/80 space-y-1 pl-2">
                ...
                {currentContentAnalysis.warnings.map(
