...
import {
  ...
  ContentAnalysis,
  ContentChunk,
} from "@/lib/contentAnalyzer"; // Ensure ContentAnalyzer v2 is used
  ...
  if (sentencesArray.length === 0 && text.trim().length > 0) {
    ...
    lines.forEach((line, idx) => {
      ...
      sentencesArray.push({
        text: line.trim(),
        start:
          globalCharOffset + (lineStartInText !== -1 ? lineStartInText : 0),
        end:
          globalCharOffset +
          (lineStartInText !== -1 ? lineStartInText : 0) +
          line.length,
        globalIndex: globalSentenceOffset + localSentenceIdx,
        localIndex: localSentenceIdx,
      });
      currentLineCharOffset =
        (lineStartInText !== -1 ? lineStartInText : 0) + line.length;
    });
    if (sentencesArray.length === 0 && text.trim().length > 0) {
      localSentenceIdx++;
      sentencesArray.push({
...
export async function POST(req: NextRequest) {
  try {
    ...
    const currentAnalysis: ContentAnalysis =
      isChunk && analysisFromClient
        ? analysisFromClient
    ...
    if (useCache) {
      ...
      if (cachedFullResponse) {
        console.log(
          `Returning cached full extraction data for ${isChunk ? `chunk ${chunkId}` : "full document"}`,
        );
        return NextResponse.json({
    ...
    if (!apiKey) {
      return NextResponse.json(
        { error: "Gemini API key is not configured (extractclaims)." },
        { status: 500 },
      );
    }
    ...
    const sentencesWithGlobalIndices = getSentencesWithIndices(
      textToProcess,
      chunkGlobalCharStartIndex,
...
async function performClaimExtractionLogic(
  ...
  const referencesInThisBlock = (
    textContentToAnalyze.match(/\[\d+\]|\(\d{4}\)|et al\.|doi:/gi) || []
  ).length;
...
Example claim object:
{
  ...
  try {
    ...
    if (jsonMatch && (jsonMatch[1] || jsonMatch[2])) {
      ...
      console.warn(
        `Could not extract JSON from Gemini response for ${currentProcessingUnitId}. Using raw text.`,
      );
    }
    text = text.trim();
    ...
    try {
      ...
      console.error(
        `Failed to parse Gemini response for ${currentProcessingUnitId}. Raw text (first 500 chars):`,
        text.substring(0, 500),
        parseError,
      );
      throw new Error(
        `Invalid JSON response from AI for claims extraction (unit: ${currentProcessingUnitId}).`,
      );
    }
    ...
    const finalResponse: ExtractionResponse = {
      topic:
        isFirstChunkOrOnlyChunk || isActualFirstChunkOfDocument
          ? parsedResponse.topic || "General (Default)"
          : "N/A (Subsequent Chunk)",
      category:
        isFirstChunkOrOnlyChunk || isActualFirstChunkOfDocument
          ? parsedResponse.category || "Other (Default)"
          : "N/A (Subsequent Chunk)",
      document_type:
        isFirstChunkOrOnlyChunk || isActualFirstChunkOfDocument
          ? parsedResponse.document_type || "Unknown (Default)"
          : "N/A (Subsequent Chunk)",
      expected_accuracy_range:
        isFirstChunkOrOnlyChunk || isActualFirstChunkOfDocument
          ? parsedResponse.expected_accuracy_range || "Unknown (Default)"
          : "N/A (Subsequent Chunk)",
      funding_source:
        isFirstChunkOrOnlyChunk || isActualFirstChunkOfDocument
          ? parsedResponse.funding_source
          : undefined,
      author_credibility:
        isFirstChunkOrOnlyChunk || isActualFirstChunkOfDocument
          ? parsedResponse.author_credibility
          : undefined,
      claims: [],
      total_sentences_in_doc:
        isFirstChunkOrOnlyChunk || isActualFirstChunkOfDocument
          ? parsedResponse.total_sentences_in_doc ||
            fullDocAnalysisContext.sentenceCount
          : 0,
      total_words_in_doc:
        isFirstChunkOrOnlyChunk || isActualFirstChunkOfDocument
          ? parsedResponse.total_words_in_doc ||
            fullDocAnalysisContext.wordCount
          : 0,
      avg_words_per_page:
        isFirstChunkOrOnlyChunk || isActualFirstChunkOfDocument
          ? parsedResponse.avg_words_per_page ||
            Math.round(
              fullDocAnalysisContext.wordCount /
                (fullDocAnalysisContext.estimatedPages || 1),
            )
          : 0,
      references_in_doc_count:
        isFirstChunkOrOnlyChunk || isActualFirstChunkOfDocument
          ? parsedResponse.references_in_doc_count || referencesInThisBlock
          : referencesInThisBlock,
      doc_reference_quality_score:
        isFirstChunkOrOnlyChunk || isActualFirstChunkOfDocument
          ? parsedResponse.doc_reference_quality_score
          : undefined,
    };

    if (parsedResponse.claims && Array.isArray(parsedResponse.claims)) {
      finalResponse.claims = parsedResponse.claims
        .filter(
          (claim) =>
            claim &&
            typeof claim.claim_id === "string" &&
            typeof claim.claim_text === "string" &&
            claim.claim_text.length > 3 &&
            claim.claim_text.length <= 250 &&
            typeof claim.original_sentence === "string" &&
            claim.original_sentence.length > 3 &&
            claim.original_sentence.length <= 500 &&
            typeof claim.sentence_number === "number" &&
            claim.sentence_number >= 0 &&
            typeof claim.sentence_start_index === "number" &&
            claim.sentence_start_index >= 0 &&
            typeof claim.sentence_end_index === "number" &&
            claim.sentence_end_index > claim.sentence_start_index &&
            claim.claim_complexity_assessment &&
            typeof claim.claim_complexity_assessment.verdict === "string" &&
            claim.claim_type_assessment &&
            typeof claim.claim_type_assessment.verdict === "string",
        )
        .map((claim) => ({
          ...claim,
          claim_text: claim.claim_text.substring(0, 250),
          original_sentence: claim.original_sentence.substring(0, 500),
          claim_complexity_assessment: claim.claim_complexity_assessment || {
            verdict: "N/A",
            reason: "Not assessed",
          },
          claim_type_assessment: claim.claim_type_assessment || {
            verdict: "N/A",
            reason: "Not assessed",
          },
        }));
      ...
      console.warn(
        `No claims array found or it's not an array in response for ${currentProcessingUnitId}.`,
      );
    }
    ...
    console.error(
      `Error in performClaimExtractionLogic for ${currentProcessingUnitId}:`,
      error,
    );
    throw error;
  }
